...
RSpec.describe Core::Category::Entity do
  include ActiveSupport::Testing::TimeHelpers

  def build_category(goal_ends_on: nil, goals: [])
    described_class.new(
      id: UUID.generate,
  ...
  def build_goal(starts_on:, value:)
    Core::Category::Goal::Entity.new(id: UUID.generate, month: starts_on.month, year: starts_on.year, value:)
  end

  it "returns nil current_goal when there are no goals" do
    expect(build_category.current_goal).to be_nil
  end
  ...
  it "returns nil current_goal after goal_ends_on" do
    ...
    travel_to(Date.new(2026, 7, 1)) do
      expect(category.current_goal).to be_nil
    end
  end
  ...
  it "falls back to the earliest goal when every goal starts in a future month" do
    ...
    category = build_category(goals: [ later, earliest ])

    travel_to(Date.new(2026, 8, 1)) do
      expect(category.current_goal).to eq(earliest)
    end
  end
end
